...
/* harmony default export */ __webpack_exports__["default"] = ({
    ...
    page: {
      bind: function bind(el, binding, vNode) {
        ...
        var _binding$value = binding.value,
            wrapperCls = _binding$value.wrapperCls,
            childWrapperCls = _binding$value.childWrapperCls,
            callback = _binding$value.callback,
            beforeCallback = _binding$value.beforeCallback,
            afterCallback = _binding$value.afterCallback,
            throttleTime = _binding$value.throttleTime,
            loadingOpen = _binding$value.loadingOpen,
            loadFirst = _binding$value.loadFirst;
        // 父容器

        var wrapperHeight = void 0;
        // 子容器
        var childNodeWrapperHeight = void 0;
        // 子节点
        ...
        if (childWrapperCls && childNode && childNode.classList.contains(childWrapperCls)) {
          if (vm.currentPage === 1 && loadFirst !== false) {
            // 初次加载
            vm.doCallback(callback, beforeCallback, afterCallback);
          }

          el.addEventListener('touchmove', vm.throttle(function () {
            wrapperHeight = el.offsetHeight;
            childNodeWrapperHeight = childNode.offsetHeight;
            if (!vm.pageLoading && !vm.noData && childNodeWrapperHeight > wrapperHeight && wrapperHeight + el.scrollTop >= childNodeWrapperHeight
  ...
  watch: {
    pageLoading: function pageLoading(val) {
  ...
  created: function created() {
    // "foobar".includes("foo")
    console.log([1].keys());
    new __WEBPACK_IMPORTED_MODULE_0_babel_runtime_core_js_promise___default.a();
  },


  methods: {
    doCallback: function doCallback(callback, beforeCallback, afterCallback) {
      var _this = this;
      ...
      callback && callback(function (data) {
        ...
        if (data instanceof Array) {
          _this.noData = data.length === 0 && _this.currentPage > 1;
          afterCallback && afterCallback(_this.noData);

          // 显示无更多了
          if (_this.noData) {
            _this.$nextTick().then(function () {
              _this.loadingShow(true, '没有更多了');
            });
          }
        } else {
    ...
    genLoading: function genLoading(parentEl) {
    ...
    loadingShow: function loadingShow(type) {
      var txt = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '加载中...';

      if (this.loadingEl) {
        this.loadingEl.innerText = txt;
        this.loadingEl.style.display = type ? '' : 'none';

        var parentEl = this.loadingEl.parentNode;
